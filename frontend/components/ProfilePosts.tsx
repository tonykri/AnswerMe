...
export default function ProfilePosts(props: any) {
    ...
    return (
        <div className="container mx-auto md:mt-10 mt-5">
            ...
            <div className="lg:h-full lg:max-h-screen dark:bg-slate-500 bg-slate-100 mt-8 py-1 lg:my-2 rounded-lg overflow-auto">
                {posts.length > 0 ? posts.map(item => (<Post key={item.id} {...item} deletePost={deletePost} viewUpdatedToast={setViewUpdated} />)) :
                    <h1 className="text-3xl my-20 text-center">{loadData}</h1>
                }
            </div>
